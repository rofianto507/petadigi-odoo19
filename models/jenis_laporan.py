import secrets
import base64
from io import BytesIO
from urllib.parse import quote
import qrcode
import qrcode.constants

from odoo import models, fields, api


class JenisLaporan(models.Model):
    _name = 'petadigi.jenis_laporan'
    _description = 'Jenis Laporan Cooling System'
    _order = 'nama asc'
    _rec_name = 'nama'

    nama = fields.Char('Nama', required=True)
    keterangan = fields.Text('Keterangan')
    state = fields.Selection([
        ('draft', 'Draft'),
        ('aktif', 'Aktif'),
        ('non_aktif', 'Non Aktif'),
    ], string='State', required=True, default='draft')
    public_token = fields.Char('Token', readonly=True, copy=False)
    public_url = fields.Char('URL Form', compute='_compute_public_url', store=False)
    qr_code_image = fields.Binary(compute='_compute_qr_code', string='QR Code', store=False)

    @api.depends('public_token')
    def _compute_public_url(self):
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        for rec in self:
            rec.public_url = f"{base_url}/giat/{rec.public_token}" if rec.public_token else ''

    @api.depends('public_url')
    def _compute_qr_code(self):
        for rec in self:
            if rec.public_url:
                qr = qrcode.QRCode(
                    version=1,
                    error_correction=qrcode.constants.ERROR_CORRECT_M,
                    box_size=6,
                    border=2,
                )
                qr.add_data(rec.public_url)
                qr.make(fit=True)
                img = qr.make_image(fill_color='#0d47a1', back_color='white')
                buf = BytesIO()
                img.save(buf, format='PNG')
                rec.qr_code_image = base64.b64encode(buf.getvalue())
            else:
                rec.qr_code_image = False

    jumlah_giat = fields.Integer(
        string='Total Giat',
        compute='_compute_jumlah_giat',
    )

    @api.depends()
    def _compute_jumlah_giat(self):
        data = self.env['petadigi.hasil_giat'].read_group(
            [('jenis_laporan_id', 'in', self.ids)],
            ['jenis_laporan_id'],
            ['jenis_laporan_id'],
        )
        counts = {d['jenis_laporan_id'][0]: d['jenis_laporan_id_count'] for d in data}
        for rec in self:
            rec.jumlah_giat = counts.get(rec.id, 0)

    def action_view_hasil_giat(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': f'Hasil Giat — {self.nama}',
            'res_model': 'petadigi.hasil_giat',
            'view_mode': 'list,form',
            'domain': [('jenis_laporan_id', '=', self.id)],
            'context': {'default_jenis_laporan_id': self.id},
        }

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('public_token'):
                vals['public_token'] = secrets.token_urlsafe(32)
        return super().create(vals_list)

    def action_set_aktif(self):
        for rec in self:
            if not rec.public_token:
                rec.public_token = secrets.token_urlsafe(32)
        self.write({'state': 'aktif'})

    def action_set_non_aktif(self):
        self.write({'state': 'non_aktif'})

    def action_set_draft(self):
        self.write({'state': 'draft'})

    def action_regenerate_token(self):
        for rec in self:
            rec.public_token = secrets.token_urlsafe(32)

    def action_share_whatsapp(self):
        self.ensure_one()
        msg = f"Form input laporan kegiatan '{self.nama}':\n{self.public_url}"
        url = f"https://web.whatsapp.com/send?text={quote(msg)}"
        return {
            'type': 'ir.actions.act_url',
            'url': url,
            'target': 'new',
        }
